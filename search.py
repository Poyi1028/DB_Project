...
@app.route('/')
def index():
    return open("search.html", "r", encoding="utf-8").read()

@app.route('/gym')
def gym():
    raw = open("gym.html", "r", encoding="utf-8").read()
...
@app.route('/equipment')
def equipment():
    raw = open("equipment.html", "r", encoding="utf-8").read()
...
@app.route('/coach')
def coach():
    raw = open("coach.html", "r", encoding="utf-8").read()
    gymOptions = "\n".join(['<option value="' + gym['Name'] + '">' + gym['Name'] + '</option>' for gym in get_data("gym")])
    ...
    return open("coach.html", "r", encoding="utf-8").read()

@app.route('/course')
def course():
    raw = open("course.html", "r", encoding="utf-8").read()
    gymOptions = "\n".join(['<option value="' + gym['Name'] + '">' + gym['Name'] + '</option>' for gym in get_data("gym")])
    ...
    return open("course.html", "r", encoding="utf-8").read()
